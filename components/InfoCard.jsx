...
const InfoCard = ({ title, description, techstack, Url }) => {
  return (
    <a href={Url} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center justify-center max-w-xs mx-auto my-4">
      ...
      <a href={Url} target="_blank" rel="noopener noreferrer" className="inline-block mt-4 px-4 py-2 text-sm font-medium leading-5 text-center text-white transition-colors duration-150 bg-gray-400 rounded hover:bg-yellow-400 focus:outline-none focus:shadow-outline-blue active:bg-blue-600">{`Take a Peek 👀`}</a>
    </a>
  );
};
